...
#Calling on submit of Attendance to apply rules based on config in HR Policy Engine
def apply_attendance_rules(doc, method):
    # Fetch all active rules
    rules = frappe.get_all("HR Policy Engine", filters={"enable": 1, "docstatus":1}, fields="*")

    # 1. Check for Daily Hours Fulfilment rule
    daily_hours_rule = next((r for r in rules if r["rule_type"] == "Daily Hours Fulfillment"), None)
    if daily_hours_rule and not doc.leave_application:
        check_daily_hours_fulfillment(doc, daily_hours_rule)
...
def apply_late_entry_rule(attendance, rule):
    ...
    if rule.affect_leave_balance:
        ...
        priorities = frappe.db.sql(
            ...
            WHERE parent = %s
            AND parenttype = 'HR Policy Engine'
...
def check_daily_hours_fulfillment(attendance, rule):
    ...
    if rule.affect_payroll_directly:
        existing_deductions = frappe.db.count(
            "Payroll Deduction Log",
            {"employee": employee, "rule_name": rule.name, "month": month_key, "docstatus": 1},
        )
